...
export class RatingsService {
	...
	static async getUsersWhoHaveRated(uuid: string) {
		try {
			const response = await axios.get(API_URL + "/rating/getraters/" + uuid, {
				headers: authHeader(),
			});
			return response;
		} catch (error) {
			console.error(
				"Error obteniendo los usuarios que han participado en un rating.",
				error
			);
			throw error;
		}
	...
	static async getRating(idRatedObject: string, ratingType: string) {
		try {
			const response = await axios.get(
				API_URL + "/rating/get/" + idRatedObject + "/" + ratingType,
				{ headers: authHeader() }
			);
			return response;
		} catch (error) {
	...
	static async insertRating(rating: RatingsEntity) {
		try {
			const response = await axios.post(API_URL + "/rating/add", rating, {
				headers: authHeader(),
			});
			return response;
		} catch (error) {
