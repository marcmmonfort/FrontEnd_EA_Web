...
export class PublicationService {
	static async feed(numPage: string, uuid: string) {
		try {
			const response = await axios.get(
				API_URL + "/followingPost/" + numPage + "/" + uuid,
				{ headers: authHeader() }
			);
	...
	static async numPublicationsFollowing(uuid: string) {
		try {
			const response = await axios.get(API_URL + "/numFollowingPost/" + uuid, {
				headers: authHeader(),
			});
			//console.log("try response " + response)
			return response;
	...
	static async updateLike(uuid: string, uuidUser: string) {
		try {
			const response = await axios.put(
				API_URL + "/parameter/like",
				{ uuid: uuid, uuidUser: uuidUser },
				{ headers: authHeader() }
			);
			//console.log("try response " + response)
			return response;
